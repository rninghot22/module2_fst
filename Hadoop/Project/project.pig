--- Load Data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/ruchika/inputs/episodeIV_dialogues.txt' USING PigStorage('\t) AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/ruchika/inputs/episodeV_dialogues.txt' USING PigStorage('\t) AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/ruchika/inputs/episodeVI_dialogues.txt' USING PigStorage('\t) AS (name:chararray, line:chararray);
...
rmf hdfs:///user/ruchika/outputs;

--- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/ruchika/outputs' USING PigStorage('\t);